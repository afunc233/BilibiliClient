...
public sealed unsafe class FFFormatContext : CountedReference<AVFormatContext>
{
        ...
        FFCodec codec)
    {
        ...
        {
            var keyParts = t.Key.Split(semicolonSeparator, 2);
    ...
    private IReadOnlyList<FFDictionary> FindStreamInfoOptions(StringDictionary codecOptions)
    {
        ...
        // }
        // 下面是 linq 简化
        result.AddRange(from stream in Streams let codecId = stream.CodecParameters.CodecId select FilterCodecOptions(codecOptions, codecId, stream, null));
